package com.example.cgz.bloodsoulnote2.imitate.slipupdown.view;
...
public class UCIndexView extends FrameLayout implements TouchMoveView.TouchMoveListener {
    ...
    public void onTouchMoveEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                int offset = 0;
                if(mDelY > 0) {//hide，下拉
                    if(!mIsPullRestoreEnable  && isShowFinish()) {//当前不允许下拉恢复
                        return;
                    }
    ...
    private void viewMove(float delY, boolean isPullRestoreEnable) {
        ...
        if(delY > 0) {//下滑
            if(!isPullRestoreEnable && isShowFinish()) {//当前不允许下拉恢复
                return;
            }
